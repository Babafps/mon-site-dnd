...
(function () {
    ...
    function renderLivePlayers() {
        ...
        el.innerHTML = live.players.map(p => {
            ...
            const seen = (!online && p.updated_at) ? timeAgo(p.updated_at) : '';
            ...
            return `<div class="gm-live-item${online ? '' : ' is-offline'}">
                <div class="gm-live-top">
                    ...
                    <span class="gm-spacer"></span>
                    <span class="gm-party-sub">${seen ? esc('⚪ vu ' + seen) : esc(sub || '—')}</span>
    ...
    function ensurePlayerModal() {
        ...
        m.addEventListener('click', (e) => {
            ...
            const u = e.target.closest('[data-act="player-unban"]');
            if (u) { unbanPlayer(u.dataset.uid); }
        });
        return m;
    }
    // Temps relatif compact ("il y a 3 min", "il y a 2 h"…)
    ...
    // Sections détaillées de la fiche (inventaire, sorts, attaques, dons, notes…).
    function renderPlayerDetails(d) {
        if (!d) return '<div class="gm-readonly-note">Le joueur n\'a pas encore partagé de fiche détaillée (ancienne version ou aucune donnée).</div>';
        const sec = (title, inner) => inner ? `<details class="gm-pm-det"><summary>${title}</summary><div class="gm-pm-det-body">${inner}</div></details>` : '';
        const pre = txt => txt ? `<div class="gm-pm-note">${esc(txt).replace(/\n/g, '<br>')}</div>` : '';
        const id = d.identity || {}, comp = d.companion || {};

        const identInner = [
            id.alignment ? `<b>Alignement :</b> ${esc(id.alignment)}` : '',
            id.languages ? `<b>Langues :</b> ${esc(id.languages)}` : '',
            id.appearance ? pre(id.appearance) : ''
        ].filter(Boolean).join('<br>');

        const invInner = (d.inventory || []).length ? (d.inventory || []).map(it =>
            `<div class="gm-pm-row"><span class="gm-pm-row-n">${esc(it.name || '?')}${(it.qty && it.qty > 1) ? ' ×' + it.qty : ''}</span>${it.category ? `<span class="gm-pm-tag">${esc(it.category)}</span>` : ''}${it.weight ? `<span class="gm-pm-row-x">${esc(it.weight)}</span>` : ''}</div>`
        ).join('') : '';

        const atkInner = (d.attacks || []).length ? (d.attacks || []).map(a =>
            `<div class="gm-pm-row"><span class="gm-pm-row-n">⚔️ ${esc(a.name || '?')}</span>${a.bonus ? `<span class="gm-pm-tag">${esc(a.bonus)}</span>` : ''}${a.dmg ? `<span class="gm-pm-row-x">${esc(a.dmg)}</span>` : ''}${a.notes ? `<div class="gm-pm-row-sub">${esc(a.notes)}</div>` : ''}</div>`
        ).join('') : '';

        const spInner = (d.spells || []).length ? (d.spells || []).slice().sort((x, y) => (x.level || 0) - (y.level || 0)).map(sp =>
            `<div class="gm-pm-row"><span class="gm-pm-row-n">${sp.prepared ? '📋 ' : ''}${esc(sp.name || '?')}</span><span class="gm-pm-tag">${(sp.level === 0 || sp.level === '0') ? 'Tour' : 'Niv ' + (sp.level != null ? sp.level : '?')}</span>${sp.range ? `<span class="gm-pm-row-x">${esc(sp.range)}</span>` : ''}</div>`
        ).join('') : '';

        const trInner = (d.traits || []).length ? (d.traits || []).map(t => {
            const tag = t.type === 'race' ? 'Espèce' : (t.type === 'feat' ? 'Don' : 'Classe');
            return `<div class="gm-pm-row"><span class="gm-pm-row-n">${esc(t.name || '?')}</span><span class="gm-pm-tag">${tag}${t.level ? ' · niv ' + t.level : ''}</span>${t.desc ? `<div class="gm-pm-row-sub">${esc(t.desc)}</div>` : ''}</div>`;
        }).join('') : '';

        const abInner = (d.abilities || []).length ? (d.abilities || []).map(a =>
            `<div class="gm-pm-row"><span class="gm-pm-row-n">${esc(a.name || '?')}</span><span class="gm-pm-tag">${a.current != null ? a.current : (a.max != null ? a.max : '?')}${a.max != null ? ' / ' + a.max : ''}</span></div>`
        ).join('') : '';

        const compInner = (comp.name || comp.notes || comp.hp || comp.ac) ?
            `<b>${esc(comp.name || 'Compagnon')}</b>${comp.ac ? ' · CA ' + esc(comp.ac) : ''}${comp.hp ? ' · PV ' + esc(comp.hp) : ''}${comp.notes ? pre(comp.notes) : ''}` : '';

        const questInner = [d.quests ? '<b>Quêtes :</b>' + pre(d.quests) : '', d.npcs ? '<b>PNJ :</b>' + pre(d.npcs) : ''].filter(Boolean).join('');

        const out = [
            sec('🪪 Identité', identInner),
            sec('🎒 Inventaire' + (invInner ? ' (' + (d.inventory || []).length + ')' : ''), invInner),
            sec('⚔️ Attaques' + (atkInner ? ' (' + (d.attacks || []).length + ')' : ''), atkInner),
            sec('✨ Sorts' + (spInner ? ' (' + (d.spells || []).length + ')' : ''), spInner),
            sec('📜 Capacités & Dons' + (trInner ? ' (' + (d.traits || []).length + ')' : ''), trInner),
            sec('🔋 Capacités limitées', abInner),
            sec('🐾 Compagnon', compInner),
            sec('🗺️ Quêtes & PNJ', questInner),
            sec('📝 Note rapide', d.quickNote ? pre(d.quickNote) : '')
        ].filter(Boolean).join('');
        return out || '<div class="gm-readonly-note">Fiche détaillée vide.</div>';
    }
    ...
    function openPlayerModal(uid) {
        ...
        const online = live.online.has(uid);
        const banUntil = live.bans[uid];
        const isBanned = banUntil && banUntil > Date.now();
        const statusLine = online
            ? '<span class="gm-pm-status on">🟢 En ligne</span>'
            : `<span class="gm-pm-status off">⚪ Hors ligne — dernier état ${esc(timeAgo(p.updated_at))}</span>`;
        const banLine = isBanned
            ? `<div class="gm-pm-ban">🚫 Banni — expire à ${esc(new Date(banUntil).toLocaleTimeString())} <button class="gm-btn" data-act="player-unban" data-uid="${esc(uid)}">Lever le ban</button></div>`
            : '';
        byId('gm-player-modal-body').innerHTML = `
            ...
            <div class="gm-pm-statusrow">${statusLine}</div>
            ${banLine}
            ...
            <div class="gm-pm-details">${renderPlayerDetails(s.details)}</div>
            <div class="gm-pm-mod">
